(function() {
    ...
    style.innerHTML = `
        #seed-overlay {
            ...
            pointer-events: all;
            overflow: hidden;
            transition: opacity 2.5s cubic-bezier(0.4, 0, 0.2, 1); /* 白幕散開的速度 */
        }
        ...
        .seed-of-light {
            ...
            cursor: pointer;
            z-index: 20001;
            transition: transform 1.2s cubic-bezier(0.7, 0, 0.3, 1); /* 炸開的力度 */
            animation: seed-breathing 3s infinite ease-in-out;
        }

        @keyframes seed-breathing {
            0%, 100% { transform: scale(1); box-shadow: 0 0 50px #fff; }
            50% { transform: scale(1.05); box-shadow: 0 0 80px #fff, 0 0 40px #d4af37; }
        }

        /* 炸開狀態：這時球體大到蓋住全螢幕，背景同步變白 */
        .seed-of-light.expand {
            transform: scale(300); 
            animation: none;
        }

        #seed-overlay.white-out {
        ...
        body.focus-in { 
            animation: web-focus 4s cubic-bezier(0.1, 0, 0.2, 1) forwards; 
        }
        @keyframes web-focus {
            0% { filter: blur(15px) brightness(2); }
        ...
        .track-item { padding: 12px; cursor: pointer; color: #888; border-bottom: 1px solid #1a1a1a; font-size: 13px; transition: 0.2s; }
    ...
    container.innerHTML = `
        ...
        <div id="playlist-window"><div style="padding:15px; color:#d4af37; font-weight:bold; border-bottom:1px solid #333; font-size:12px;">LIBRARY AUDIO</div><div id="playlist-content" style="max-height: 300px; overflow-y: auto;"></div></div>
    ...
    window.onYouTubeIframeAPIReady = function() {
        player = new YT.Player('youtube-player', {
            ...
            events: { 'onReady': () => { document.getElementById('start-btn').onclick = startFocusRitual; initUI(); } }
        });
    };
    ...
    function transitionVolume(start, end, duration) {
        ...
        function update() {
            ...
            if (progress < 1) requestAnimationFrame(update); else isTransitioning = false;
        }
    ...
    function startFocusRitual() {
        ...
        }, 800); // 這個延遲是關鍵，要對準 expand 動畫的中後段

        setTimeout(() => showNotice(tracks[currentTrackIndex].name), 3500);
    }

    function initUI() {
        document.getElementById('music-control-btn').onclick = (e) => { e.stopPropagation(); document.getElementById('playlist-window').classList.toggle('open'); };
